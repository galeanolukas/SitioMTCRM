...
$(function () {
  ...
  $("input[name='iva']").TouchSpin({
    ...
    if (!isNaN(n)) localStorage.setItem('iva_rate', n.toString());
    vents.calculate_invoice();
  }).val(storedIva).trigger('change');
  // event submit
  ...
  $('form').on('submit', function (e) {
    ...
    submit_with_ajax(window.location.pathname, 'Alerta!', '¿Estas seguro de realizar la siguiente acción?', parameters, function () {
      var redirect = '/erp/sale/list/';
      if ($('input[name="action"]').val() === 'add_invoice') {
        redirect = '/erp/invoice/list/';
      }
